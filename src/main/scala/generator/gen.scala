...
object Generator extends App {
  ...
  modset_countries("United States").probabilityTrendPeak = 0.25f;
  ...
  def getCountry() : Mod = {
    ...
    if (Random.nextFloat() < 0.008f) { modset_countries.mods(0).startTrend(); }
  ...
  def generateTool() : Product = {
    ...
    if (Random.nextFloat() < 0.008f) { 
      modset_tool_materials.mods(0).startTrend(); 
      modset_tool_materials.mods(1).startTrend(); 
    }
  ...
  modset_clothing_materials("Cotton").setPriceTrendInfo(1f, 2f);
  modset_clothing_materials("Cotton").setProbabilityTrendInfo(0.6f, 0.3f);
  modset_clothing_materials("Denim").setPriceTrendInfo(1.8f, 2.4f);
  ...
  def generateClothing() : Product = {
    ...
    if (Random.nextFloat() < 0.008f) { 
      modset_clothing_materials.mods(0).startTrend(); 
      modset_clothing_materials.mods(1).startTrend(); 
    }
  ...
  modset_electronic_types("Laptop Computer").setPriceTrendInfo(6f, 12f);
  modset_electronic_types("Laptop Computer").setProbabilityTrendInfo(0.143f, 0.4f);

  def generateElectronic() : Product = {
    
    // LG brand popularity increase
    if (Random.nextFloat() < 0.008f) { modset_electronic_brands.mods(0).startTrend(); }
    // Dell brand popularity increase
    if (Random.nextFloat() < 0.008f) { modset_electronic_brands.mods(1).startTrend(); }
    // laptop computer popularity increase
    if (Random.nextFloat() < 0.008f) { modset_electronic_types.mods(0).startTrend(); }
  ...
  modset_payment_types("Failure").setProbabilityTrendInfo(0.05f, 0.05f)
