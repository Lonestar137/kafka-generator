...
object Generator extends App {
  ...
  var modset_countries : ModSet = new ModSet("Countries",
  Mod("United States"),
  Mod("China"),
  Mod("United Kingdom"),
  Mod("Japan"),
  ...
  def generateCustomer() : Customer = {
    ...
    var customerCountry : Mod = getCountry();
    newCustomer.country_=(customerCountry.useName());
    newCustomer.city_=(customerCountry.randomSubstring());
  ...
  modset_tool_materials.mods(0).setProbabilityTrendInfo(0.4f, 0.2f);
  modset_tool_materials.mods(1).setProbabilityTrendInfo(0.33f, 0.66f);
  modset_tool_materials.mods(2).setProbabilityTrendInfo(0.1f, 0.1f);
  
  def generateTool() : Product = {
    
    // material trend steel demand spike
    if (Random.nextFloat() < 0.008f) { 
      modset_tool_materials.mods(0).startTrend(generatorDate); 
      modset_tool_materials.mods(1).startTrend(generatorDate); 
    }
  ...
  modset_clothing_materials.mods(0).setPriceTrendInfo(1f, 2f);
  modset_clothing_materials.mods(0).setProbabilityTrendInfo(0.6f, 0.3f);
  modset_clothing_materials.mods(1).setPriceTrendInfo(1.8f, 2.4f);
  modset_clothing_materials.mods(1).setProbabilityTrendInfo(0.3f, 0.6f);
  modset_clothing_materials.mods(2).setProbabilityTrendInfo(0.1f, 0.2f);

  def generateClothing() : Product = {
    
    // material trend cotton shortage / denim demand spike
    if (Random.nextFloat() < 0.008f) { 
      modset_clothing_materials.mods(0).startTrend(generatorDate); 
      modset_clothing_materials.mods(1).startTrend(generatorDate); 
    }
  ...
  val modset_electronic_types : ModSet = new ModSet("Electronics Types", 
    Mod("Laptop Computer", 6f), 
    Mod("Desktop Computer", 7f), 
    Mod("Printer", 2f), 
    Mod("Television", 3.5f), 
    Mod("Smartphone", 4f), 
    Mod("3D Printer", 5f), 
  ...
  modset_electronic_types.setModProbabilities(0.143f);
  modset_electronic_types.setTrendInfo("Laptop Computer", 6f, 12f, 0.143f, 0.4f);

  def generateElectronic() : Product = {
    
    // LG brand popularity increase
    if (Random.nextFloat() < 0.008f) { modset_electronic_brands.mods(0).startTrend(generatorDate); }
    // Dell brand popularity increase
    if (Random.nextFloat() < 0.008f) { modset_electronic_brands.mods(1).startTrend(generatorDate); }
    // laptop computer popularity increase
    if (Random.nextFloat() < 0.008f) { modset_electronic_types.mods(0).startTrend(generatorDate); }
  ...
  modset_payment_types.setModProbabilities(0.2375f);
  modset_payment_types.setProbabilityTrendInfo("Failure", 0.05f, 0.05f)
  ...
  def generateOrderJSON(paymentType: String, transactionSuccess: Boolean = true, transactionFailureReason: String = "") : String = {
    ...
    output = output + "\t\"payment_type\" : \"" + paymentType + "\",\n";
    ...
    output = output + "\t\"payment_txn_success\" : " + transactionSuccess + ",\n";
    output = output + "\t\"failure_reason\" : \"" + transactionFailureReason + "\"\n";
    output = output + "}";

    return output.replace("\t","").replace("\n","");
    //return s"[${order.order_id} ${dateFormat.format(order.order_datetime)}] ${product.product_name} ($$${product.product_price}) ${customer.country}";
  ...
  var paymentFailureReasonLists = collection.mutable.Map[String, MutableList[String]]();
  var nextPaymentFailure : String = "";
  //TODO more, or more specific payment failure reasons
  ...
  paymentFailureReasonLists += ("Card" -> cardPaymentFailures);
  ...
  paymentFailureReasonLists += ("Internet Banking" -> internetBankingPaymentFailures);
  ...
  paymentFailureReasonLists += ("UPI" -> upiPaymentFailures);
  ...
  paymentFailureReasonLists += ("Wallet" -> walletPaymentFailures);
